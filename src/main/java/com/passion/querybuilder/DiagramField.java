...
import com.passion.querybuilder.dnd.DragMouseAdapter;
import com.passion.querybuilder.dnd.RelationDropTargetListener;
import com.passion.querybuilder.dnd.RelationTransferHandler;
...
public class DiagramField extends JPanel implements ItemListener, MouseListener, PopupMenuListener
{
	...
	private class ActionAddExpression extends AbstractAction
	{
		...
		{
			final String ZigmaDataGroupConcat = "ZigmaDataGroupConcat";
			final List<String> functions = new ArrayList<String>(Arrays.asList(SQLHelper.SQL_AGGREGATES));
			functions.add(ZigmaDataGroupConcat);
			...
			JCheckBox pivotCheckbox = new JCheckBox(I18n.getString("querybuilder.message.ZigmaDataPivotCheckbox", "Transform as Pivot"));
			...
			{
				String expr = null;
				if(choose.toString().equals(ZigmaDataGroupConcat)){
					Object ZigmaDataGroupConcatSeparator =
						JOptionPane.showInputDialog(
							DiagramField.this.getOwner().builder,
							I18n.getString("querybuilder.message.ZigmaDataGroupConcat.chooseSeparator", "choose separator:"),
							I18n.getString("querybuilder.menu.add", "add..."),
							JOptionPane.PLAIN_MESSAGE,
							null,
							null,
							null);
					if(null == ZigmaDataGroupConcatSeparator){
						return;
					}
					if(!ZigmaDataGroupConcatSeparator.toString().startsWith("'")){
						ZigmaDataGroupConcatSeparator = "'"+ZigmaDataGroupConcatSeparator+"'";
					}
					expr = ZigmaDataGroupConcat + "(" + DiagramField.this.querytoken.getTable() +","+
							DiagramField.this.querytoken.getIdentifier() + "," + ZigmaDataGroupConcatSeparator
							+ ")";
				}else{
					//pivot
					if(pivotCheckbox.isSelected()){
						Object ZigmaDataPivotHeader =
							JOptionPane.showInputDialog(
								DiagramField.this.getOwner().builder,
								I18n.getString("querybuilder.message.ZigmaDataPivotHeaderColumn", "Choose header column for Pivot:"),
								I18n.getString("querybuilder.menu.add", "add..."),
								JOptionPane.PLAIN_MESSAGE,
								null,
								null,
								null);
						if(null == ZigmaDataPivotHeader){
							return;
						}
						expr = "ZigmaDataPivot(" + DiagramField.this.querytoken.getTable() +","+ZigmaDataPivotHeader
