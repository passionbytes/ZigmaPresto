...
public abstract class QueryTokens {
	...
	public static class DefaultExpression implements _Expression, _Alias {
		...
		public String toString() {
			return isAliasSet() ? value + SQLFormatter.SPACE + _ReservedWords.AS + SQLFormatter.SPACE + this.getAlias()
					: value;
		}
	...
	public static class Column extends AbstractDatabaseObject implements _Expression {
		...
		public String getIdentifier() {
			...
			if (owner != null)
				identifier = owner.getReference() + SQLFormatter.DOT + identifier;
			return identifier;
		}
	...
	public static class Table extends AbstractDatabaseObject implements _TableReference {
		...
		public String getSchema() {
		 ...
		 public void setSchema(String schema)
		    ...
		    public void setSchema(String schema, String catalog)
		    {
		     // String catalogName = (String)ConnectionHandler.schemaCat.get(schema);
		     // this.schema = (catalogName + "." + schema);
                if (schema.matches(IGNORE) && !(OracleDefaultUsers.getDefault_users().contains(schema))) {
		...
		/*public void setSchema(String schema) {
			...
			System.out.println("schema : " + schema);
			// is the schema from ZigmaDataQB drivere?

			if (schema.matches(IGNORE)) {
				...
				if (null != driverName && driverName.contains("ZigmaDataQB")) {
					this.schema = catalogName + "." + schema;
				} else {
					this.schema = schema;
				}
	...
	public static class Join  implements _TableReference {
		...
		public static int getTypeInt(String type) {
			if (type.toUpperCase().equals(_ReservedWords.LEFT_OUTER_JOIN)
					|| type.toUpperCase().equals(_ReservedWords.LEFT_JOIN))
				return LEFT_OUTER;
			if (type.toUpperCase().equals(_ReservedWords.RIGHT_OUTER_JOIN)
					|| type.toUpperCase().equals(_ReservedWords.RIGHT_JOIN))
				return RIGHT_OUTER;
			if (type.toUpperCase().equals(_ReservedWords.FULL_OUTER_JOIN)
					|| type.toUpperCase().equals(_ReservedWords.FULL_JOIN))
				return FULL_OUTER;
		...
		public String toString() {
			return getPrimary().getTable().toString() + SQLFormatter.SPACE + this.getTypeName() + SQLFormatter.SPACE
					+ getForeign().getTable().toString() + SQLFormatter.SPACE + _ReservedWords.ON + SQLFormatter.SPACE
					+ this.getCondition();
		}
