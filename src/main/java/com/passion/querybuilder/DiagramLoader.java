...
import com.passion.environment.io.ManualTableMetaData;
...
public class DiagramLoader extends JDialog implements Runnable {
	...
	public static void run(int mode, QueryBuilder builder, QueryTokens.Table table, boolean autojoin) {
		DiagramLoader loader = null;
          if(null != table){
       		...
       		String [] tableName = table.toString().split("\\.");
       	
       	
       		
       		if(tableName.length == 3) {
       			
       			 table.setName(tableName[2]);
       			
       		} else if(tableName.length == 2) {
       			table.setName(tableName[1]);
       		}else{
       			table.setName(tableName[0]);
       		}
           }
	...
	public void run() {
		try {
			switch (mode) {
			case ALL_FOREIGN_TABLES:
				addAllForeignTables();
				break;
			case ALL_PRIMARY_TABLES:
				addAllPrimaryTables();
				break;
			default:
				addTable(table);
	...
	private void addTable(QueryTokens.Table table) throws SQLException {
		message.setText(I18n.getFormattedString("querybuilder.message.loading", "Loading: {0}",
				new Object[] { "" + table.getIdentifier() }));
		boolean tableExists = checkTable(table);

		// fix #78 do not autoalias fields in subqueries
		// if(( QueryBuilder.autoAlias || (builder.browser.getQueryItem()
		// instanceof BrowserItems.DiagramQueryTreeItem)) &&
		// table.getAlias()==null)
		if (QueryBuilder.autoAlias && table.getAlias() == null) {
			...
			if (mode == DEFAULT) {
				...
				JOptionPane.showMessageDialog(this,
						I18n.getString("querybuilder.message.tableLoadedAliasDisabled",
								"Table already loaded and aliasing disabled!"),
						table.getIdentifier(), JOptionPane.WARNING_MESSAGE);
			}
			return;
		}
		boolean added = false;
		if (!QueryBuilder.autoAlias) {
			...
			// Ram's cahnges
			if (dbmd.getDriverName().toLowerCase().contains("presto")) {
				if(null != schema){
					String [] schemas = schema.toString().split("\\.");
					
					if(schemas.length == 2) {
						schema = schemas[1].trim();
						
					} else if(schemas.length == 1) {
						schema = schemas[0];
					}
					
				catalog = ConnectionHandler.schemaCat.get(schema) == null ? null
						: ConnectionHandler.schemaCat.get(schema);
				}
			} else {
				catalog = schema == null ? null : dbmd.getConnection().getCatalog();
			}
				...
				if (mtd != null) {
					...
					if (!added) {
						...
						for (String alias : impAliases) {
							...
							QueryTokens.Table tablex = new QueryTokens.Table(noSchema ? null : qt.getSchema(),
									qt.getName());
						...
						for (String alias : expAliases) {
							QueryTokens.Table tablex = new QueryTokens.Table(noSchema ? null : table.getSchema(),
									table.getName());
							tablex.setAlias(alias);
							createAndJoin(tablex, true);
							added = true;
						...
						if (added && woAliasExists) {
							JOptionPane.showMessageDialog(this,
									I18n.getString("querybuilder.message.tableAliasedCorrectJoin",
											"Same table added with an alias,please choose the expected one and corret joins accordingly!"),
									table.getIdentifier(), JOptionPane.WARNING_MESSAGE);
							return;
						}
					} else {
	...
	private void createAndJoin(QueryTokens.Table table, boolean tableExists) throws SQLException {
		...
		// #157
		if (Application.isFullVersion() || builder.diagram.getEntities().length < 25) {
			builder.diagram.addEntity(item);
		} else {
			JOptionPane.showMessageDialog(this,
					I18n.getString("querybuilder.message.VersionWith3tablesMax",
							"Version with 3 tables max per graph, Please Donate for more"),
					table.getIdentifier() + " " + table.getAlias(), JOptionPane.WARNING_MESSAGE);
			return;
		}

		/*
		 * lo deve fare se provengo da: click su browser, da reference o da open
		 * all non lo deve fare se sto facendo setQueryModel!
		 */
	...
	private void addTables(ArrayList mtdKeys, int rsSchemaIndex, int rsTableIndex) throws SQLException {
		...
		DatabaseMetaData dbmd = builder.getConnection().getMetaData();
		String schema = builder.getQueryModel().getSchema() == null ? table.getSchema()
				: builder.getQueryModel().getSchema();
		String catalog = ConnectionHandler.schemaCat.get(schema) == null ? null
				: ConnectionHandler.schemaCat.get(schema);
		System.out.println("***284***" + catalog);
		...
		for (Object rows : mtdKeys) {
			...
			if (builder.getQueryModel().getSchema() != null)
				schemaName = null;
			if (schemaName != null)
				schemaName = catalog.trim() + "." + schemaName.trim();
	...
	private void addTables(ResultSet rs, int rsSchemaIndex, int rsTableIndex) throws SQLException {
		...
		String catalog = ConnectionHandler.schemaCat.get(schema) == null ? null
				: ConnectionHandler.schemaCat.get(schema);
		System.out.println("***315***" + catalog);
		while (rs.next()) {
	...
	private void addAllForeignTables() throws SQLException {
		...
		System.out.println("***343***" + catalog);
		ManualDBMetaData md = ConnectionAssistant.getManualDBMetaData(builder.getConnectionHandlerKey());
		if (md != null) {
			ManualTableMetaData mtd = md.getManualTableMetaData(catalog, schema, table.getName());
			if (mtd != null) {
	...
	private void addAllPrimaryTables() throws SQLException {
		...
		String schema = builder.getQueryModel().getSchema() == null ? table.getSchema()
				: builder.getQueryModel().getSchema();
		String catalog = builder.getCatalog(schema) == null ? null : builder.getCatalog(schema);
	...
	private boolean checkTable(QueryTokens.Table table) throws SQLException {
		DatabaseMetaData dbmd = builder.getConnection().getMetaData();
		String catalog = null;
		String name = table.getName();
		String schema = builder.getQueryModel().getSchema() == null ? table.getSchema()
				: builder.getQueryModel().getSchema();
		if (builder.getConnection().getMetaData().getDriverName().toLowerCase().contains("presto")) {
			catalog = ConnectionHandler.schemaCat.get(schema) == null ? null : ConnectionHandler.schemaCat.get(schema);
		} else {
		...
		if (!exists) {
			...
			if (exists = rs.next()) {
				...
				table.setName(name);
				System.out.println("builder.getQueryModel().getSchema() == null" +builder.getQueryModel().getSchema());
				if (builder.getQueryModel().getSchema() == null)
					table.setSchema(catalog.trim() + "." + schema);
			}
			rs.close();
		}
		// RAVi at 410
		...
		// {
		// Application.alert(Application.PROGRAM,"Object " +
		// table.getIdentifier() + " doesn't exists!");
		// }
		return exists;
	}

	private DiagramEntity creatEntity(QueryTokens.Table table, boolean tableExists) throws SQLException {
		DiagramEntity item = new DiagramEntity(builder, table);
		if (!tableExists) {
			item.setFontColorAndToolTip(Color.BLUE, table.getName());
		}
		...
		if (builder.getConnection() != null) {
			...
			String schema = builder.getQueryModel().getSchema() == null ? item.getQueryToken().getSchema()
					: builder.getQueryModel().getSchema();
			
			//Ram's change
			String [] table1 = schema.toString().split("\\.");
			String tab  =null;
			...
			String catalog = null;
			if (builder.getConnection().getMetaData().getDriverName().toLowerCase().contains("presto")) {
				catalog = ConnectionHandler.schemaCat.get(tab) == null ? null: ConnectionHandler.schemaCat.get(tab);
			} else {

				catalog = tab == null ? null : dbmetadata.getConnection().getCatalog();
			}
			System.out.println("***442***" + catalog);
			ResultSet rsColumns = dbmetadata.getColumns(catalog, tab, name, "%");
	...
	private Hashtable getPrimaryKeys(DatabaseMetaData dbmetadata, DiagramEntity item) {
		...
		try {
			...
			String catalog = null;
			// Ram's cahnges
			if (dbmetadata.getDriverName().toLowerCase().contains("presto")) {
				catalog = ConnectionHandler.schemaCat.get(schema) == null ? null
						: ConnectionHandler.schemaCat.get(schema);
			} else {
				catalog = schema == null ? null : dbmetadata.getConnection().getCatalog();
			}
	...
	private void doAutoJoin(DiagramEntity source) throws SQLException {
		if (builder.diagram.getEntities().length > 1) {
			...
			String schema = builder.getQueryModel().getSchema() == null ? source.getQueryToken().getSchema()
					: builder.getQueryModel().getSchema();
			String catalog = null;
			if (builder.getConnection().getMetaData().getDriverName().toLowerCase().contains("presto")) {
				catalog = ConnectionHandler.schemaCat.get(schema) == null ? null
			...
			message.setText(I18n.getFormattedString("querybuilder.message.loading.relations", "check {0}'s relations ",
					new Object[] { "" + table.getIdentifier() }));
	...
	private void joinInternal(String pkschema, String pktable, String pkcolumn, String fkschema, String fktable,
			String fkcolumn, String fkname, String pktAlias, DiagramEntity source, boolean ispk, String joinType) {
		if (builder.getQueryModel().getSchema() != null)
		...
		if (itemP != null && itemF != null
				&& !itemP.getQueryToken().toString().equalsIgnoreCase(itemF.getQueryToken().toString())) {
			...
			boolean go = false;
			if (pktAlias == null && pktAlias == tAlias) {
				go = true;
			}
			if (pktAlias != null && pktAlias.equals(tAlias)) {
				go = true;
			}
			...
			if (go) {
				...
				if (null == fP) {
					JOptionPane.showMessageDialog(this,
							I18n.getString("querybuilder.message.noColumnExistsInTable",
									"No column: " + pkcolumn + " exists in table: " + pktable + "!"),
							table.getIdentifier(), JOptionPane.WARNING_MESSAGE);
					return;
				}
				if (null == fF) {
					JOptionPane.showMessageDialog(this,
							I18n.getString("querybuilder.message.noColumnExistsInTable",
									"No column: " + fkcolumn + " exists in table: " + fktable + "!"),
							table.getIdentifier(), JOptionPane.WARNING_MESSAGE);
					return;
				}
