...
import com.passion.querybuilder.BrowserItems.AbstractQueryTreeItem;
import com.passion.querybuilder.syntax.DerivedTable;
import com.passion.querybuilder.syntax.QueryExpression;
import com.passion.querybuilder.syntax.QuerySpecification;
import com.passion.querybuilder.syntax.QueryTokens;
import com.passion.querybuilder.syntax.SQLFormatter;
import com.passion.querybuilder.syntax.SubQuery;
...
public class ViewBrowser extends BorderLayoutPanel implements TreeSelectionListener {
	...
	private void add(int idx, QueryTokens._Base token) {
		...
		if (token instanceof QueryTokens.Join)
			...
			if (sub.isAliasSet())
				item.add(qi = new BrowserItems.QueryTreeItem(label + SQLFormatter.SPACE + sub.getAlias(),
						(SubQuery) token));
			else
				item.add(qi = new BrowserItems.QueryTreeItem(label, (SubQuery) token));
		} else if (token instanceof QueryTokens.Condition
				&& ((QueryTokens.Condition) token).getRight() instanceof SubQuery)
	...
	private void remove(int idx, QueryTokens._Base token) {
		...
		} else if (token instanceof SubQuery || (token instanceof QueryTokens.Condition
				&& ((QueryTokens.Condition) token).getRight() instanceof SubQuery)) {
	...
	void addFromClause(QueryTokens._TableReference token) {
		QueryTokens._TableReference token_ = token;
		if (builder.isLoading())
			return;
		...
		String [] table = token.toString().split("\\.");
		String tab  =null;
		
		if(table.length == 3) {
			tab = table[2];
			String [] y = tab.split("\\s");
		...
		// String schema =(String)( (QueryTokens.Table)token).getSchema();
		
		if (null != tab) {
			String schema = ConnectionHandler.schemaTab.get(tab);
			...
			((QueryTokens.Table) token).setSchema(schema, catalog);
			}
		}
	...
	void removeSelectList(QueryTokens._Expression token) {
		...
		if (token instanceof SubQuery) {
			BrowserItems.AbstractQueryTreeItem itemSub = (BrowserItems.AbstractQueryTreeItem) tree.getSelectionPath()
					.getLastPathComponent();
			BrowserItems.AbstractQueryTreeItem itemQry = (BrowserItems.AbstractQueryTreeItem) itemSub.getParent()
					.getParent();
	...
	void removeWhereClause(QueryTokens.Condition token) {
		...
		if (token.getRight() instanceof SubQuery
				&& tree.getSelectionPath().getLastPathComponent() instanceof BrowserItems.AbstractQueryTreeItem) {
			BrowserItems.AbstractQueryTreeItem itemSub = (BrowserItems.AbstractQueryTreeItem) tree.getSelectionPath()
					.getLastPathComponent();
			BrowserItems.AbstractQueryTreeItem itemQry = (BrowserItems.AbstractQueryTreeItem) itemSub.getParent()
					.getParent();
	 ...
	 * non puntare direttamente la variabile, ma usare il metodo getQueryItem!
	 * -------------------------------------------------------------------------
	 * ---------------------------------------------
	 */
	...
	private void onLoad(BrowserItems.AbstractQueryTreeItem qi) {
		...
		if (qi.getParent() != null && qi.getParent().getParent() != null) {
			queryItem = qi.getParent().getParent() instanceof AbstractQueryTreeItem
					? (AbstractQueryTreeItem) qi.getParent().getParent() : null;
		} else {
			queryItem = null;
		}
	}
