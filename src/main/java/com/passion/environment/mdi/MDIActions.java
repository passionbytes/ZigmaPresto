...
import com.passion.environment._Constants;
import com.passion.environment.ctrl.content.AbstractActionContent;
import com.passion.environment.ctrl.define.TableMetaData;
import com.passion.environment.io.FileHelper;
import com.passion.environment.io.FileStreamSQL;
...
public abstract class MDIActions implements _Constants
{
			...
			if(!ConnectionAssistant.getHandlers().isEmpty())
			{
				...
				if(keycah != null)
				{
					...
					if(!Preferences.getBoolean("querybuilder.use-schema"))
					{
						...
						if(schemas.size()>0)
						{
							Object schema = JOptionPane.showInputDialog(Application.window,I18n.getString("application.message.schema","Schema:"),
									Application.PROGRAM,JOptionPane.PLAIN_MESSAGE,null,schemas.toArray(),null);
							if(schema == null) return;
							
							System.out.println("MDIACTIONS schema: "+  schema.toString());
	...
	public static class LoadQuery extends AbstractBase
	{
		...
		{
			System.out.println("MDI LoadQuery setSchema(schema,qe.getUnion()); schema: "+  schema.toString());
			...
			System.out.println("MDI setSchema(schema,qe.getUnion()); schema: "+  schema.toString());
		...
		public void actionPerformed(ActionEvent ae)
		{
			...
			if(ret[0]!=null && ret[1]!=null && ret[2]!=null)
			{
				...
				System.out.println("MDI actionPerformed schema: "+  ret[3].toString());
		...
		private void loadFileInQueryBuilder(String fileName, Object keycah) {
			...
			if(!Preferences.getBoolean("querybuilder.use-schema"))
			{
				...
				if(schemas.size()>0)
				{
					...
					System.out.println("MDI loadFileInQueryBuilder schema: "+  schema.toString());
	...
	/*public static class Wrangle extends AbstractBase
	{
		public Wrangle(){
			super(I18n.getString("application.menu.wrangle","Wrangle"));
			setEnabled(true);
		...
		{
			try {
			ProcessBuilder pb = new ProcessBuilder("java", "-jar", "DataCleaner.jar");
	...
	public static class Visualize extends AbstractBase
	{
		public Visualize(){
			super(I18n.getString("application.menu.visualize","Visualize"));
			//setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_V,InputEvent.CTRL_MASK));
			//setText(I18n.getString("application.menu.visualize","Visualize"));
		...
		public void actionPerformed(ActionEvent ae)
		{
			if (Desktop.isDesktopSupported()) {
		        ...
		        if (desktop.isSupported(Desktop.Action.BROWSE)) {
		            try {
		                desktop.browse(new URI("http://localhost:4000"));
		            }
