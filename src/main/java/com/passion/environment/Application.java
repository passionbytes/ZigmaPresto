/*
 *
 * Modified by ZigmaData Visual Query Builder :: java database frontend with join definitions
...
import com.passion.common.util.Appearance;
import com.passion.common.util.I18n;
import com.passion.common.util.Resources;
import com.passion.common.util.Store;
import com.passion.common.util.Text;
import com.passion.environment.ctrl.commands.AliasCommand;
import com.passion.environment.ctrl.commands.Command;
import com.passion.environment.ctrl.commands.CommandRunner;
import com.passion.environment.mdi.ClientMetadataExplorer;
import com.passion.environment.mdi.DialogPreferences;
import com.passion.environment.mdi.MDIWindow;
...
public class Application extends Appearance implements _Constants,_Version
{
	...
	boolean Icon_scaling = false;
		...
		resources.loadIcon(ICON_ZigmaData,"/images/ZigmaData.png");
	...
	private static void loadSession()
	{
		try
		{
	    	...
	    	if(Preferences.getBoolean("application.trace",true))
	    	{
		    	String temp = System.getProperty("java.io.tmpdir");
		    	println("Temp dir in use: " + temp);
				System.setOut(new PrintStream(new FileOutputStream(new File(temp,"zigmadata.out"))));
				System.setErr(new PrintStream(new FileOutputStream(new File(temp,"zigmadata.err"))));
	    	}
	...
	{
		final String message = I18n.getString("application.message.quit","Do you really want to quit PrestoQB?");
	...
	//.ZigmaData file is the old file used , now replaced by sessionXMLFileName method .ZigmaData.xml
	@Deprecated
	private static String sessionFilename()
	{
		return System.getProperty("user.home") + File.separator + ".zigmadata";
	}
	
	private static void initConfigFolder(){
		final String userDir = System.getProperty("user.home");
		final File configDir = new File(userDir, ".zigmadata.config");
		if(!configDir.exists()){
			configDir.mkdir();
	...
	{
		return  CONFIG_FOLDER + File.separator + ".zigmadata.xml";
	}
	
	private static String sessionMetaviewFilename()
	{
		return  CONFIG_FOLDER + File.separator + ".zigmadata.metaview";
	}
	
	private static String sessionSQLHistoryFilename(){
		return  CONFIG_FOLDER + File.separator + ".zigmadata.history.xml";
	}
    ...
    {    	
        ...
        javax.swing.UIManager.put("OptionPane.minimumSize",new Dimension(100,100));
        if(System.getProperty("com.passion.laf.class") != null)
        {
            try
            {
                javax.swing.UIManager.setLookAndFeel(System.getProperty("com.passion.laf.class"));
        	...
        	// By default, ZigmaData uses  Metal LAF instead of OS native LAF  
        	try
            {
        ...
        System.out.println("UsingLookAndFeel: "+javax.swing.UIManager.getLookAndFeel());
		...
		//URL imgUrl = ClassLoader.getSystemResource("images/ZigmaData.png");
		// check if compatible with PNSPlugin 
        //URL imgUrl = Resources.class.getResource("/images/ZigmaData.png");
		Application.window.setIconImage(Application.resources.getIcon(Application.ICON_ZigmaData).getImage());
		try {
		...
		if(Preferences.getBoolean(DialogPreferences.CHECK_FOR_UPDATE_KEY, true)){
			// add version link on menu bar after window loading
