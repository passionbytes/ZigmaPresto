...
import com.passion.common.util.DataComparerPanelConfig;
...
import com.passion.environment.ctrl.DataComparer;
import com.passion.environment.ctrl.comparer.data.DataComparerDialogTable.DATA_TYPE;
import com.passion.environment.ctrl.editor.Task;
import com.passion.environment.ctrl.editor._TaskSource;
import com.passion.environment.ctrl.editor._TaskTarget;
import com.passion.environment.mdi.MDIClient;
import com.passion.environment.mdi._ConnectionListener;
...
public class DataComparerCriteriaPane extends JPanel implements _ConnectionListener, ItemListener, _TaskSource {
	...
	private Map<DATA_TYPE, DataComparerCriteriaDialogPane> dataTypePanelMap = new HashMap<DATA_TYPE, DataComparerCriteriaDialogPane>(
			3);
	...
	private void initComponents(final String headerText) {
		...
		gbc.gridx = 0;
		gbc.gridy = 1;
		final JLabel useConnections = new JLabel(
				I18n.getString("application.message.useConnection", "Use connection:"));
		...
		// System.out.println("schemas:DataComparerCriteriaPane : " +schemas);
		add(schemas);
		...
		if (isSource) {
			...
			gbc.gridx = 0;
			gbc.gridy = 12;
			final JTextArea label = new JTextArea(I18n.getString("datacomparer.RightSideText",
					"Note: Right side texts are automatically\ncopied from left when empty!"));
		...
		final Thread t = new Thread(new Runnable() {
			...
			private void setTextIfEmpty(final DATA_TYPE dataType) {
				...
				if (null == textS || textS.isEmpty()) {
					owner.getTarget().getDataTypePanelMap().get(dataType)
							.setText(owner.getSource().getDataType(dataType));
				}
			}
		});
		t.start();
	...
	public void setQuery() {
		...
		if (selectAppended) {
			...
			final String tableFinalName = getSchema() != null ? getSchema() + "." + getTable() : getTable();
			System.out.println("tableFinalName" + tableFinalName);
	...
	private void addDataComparerDialogPane(GridBagConstraints gbc, GridBagLayout gbl, DATA_TYPE dataType) {
		final DataComparerCriteriaDialogPane panel = new DataComparerCriteriaDialogPane(dataType, this);
	...
	public String getSchema() {
		String schema = cbxSchema.getSelectedIndex() != -1
				? (!cbxSchema.getSelectedItem().toString().isEmpty() ? cbxSchema.getSelectedItem().toString() : null)
				: null;
		if (null != schema) {
			// Ram's changes
			String[] table = schema.split("\\.");
			...
			if (null != tab) {
				...
				if (schema.matches(IGNORE) && !(OracleDefaultUsers.getDefault_users().contains(schema))) {
					...
					String catalog = ConnectionHandler.schemaCat.get(schema);
	...
	public String[] getTableColumns() {
		...
		if (null != getSchema()) {
			...
			System.out.println("Schema:; " + tab);
		}
		System.out.println("getTable(): " +getTable());
		return SQLHelper.getColumns(getHandlerKey(), tab, getTable());
	}
	...
	public void retrieveData(final PrintStream stream) {
		...
		final _TaskTarget target = new _TaskTarget() {
			...
			public void processResult(ResultSet rs) {
				try {
					...
					Object[] vals = null;
					int rowCount = 0;
					// fetch size optimisation (mainly for Oracle that as a
					// default fetch size of 10)
					...
					while (rs.next()) {
						...
						if (!isFullVersion) {
							...
							if (rowCount > LIMITED_ROWS_FOR_FREE_VERSION) {
								Application.alert(I18n.getString("datacomparer.VersionWith100rowsMax",
										"Version with max 100 rows for data comparer query result, Please Donate for more"));
								break;
							}
						}
					}
		...
		Application.session
				.addSQLToHistory(new SQLHistoryData(new Date(), getHandlerKey(), "DataComparer", getSyntax()));
	...
	public DataComparerPanelConfig getSetup() {
		...
		System.out.println(" DataComparerCriteriaPane Schema:" + getSchema());
